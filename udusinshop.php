        ...
        <div class="col-4">
            <div class="form_add">
                ...
                <div class="form_element">
                    ...
                    <input type="text" name="gmail" class="w-75 rounded border-success-subtle" id="" placeholder="Nhập email hoặc số điện thoại" value="<?php echo $row['email'] ?>" required>
                </div>
                <div class="form_element">
...
if(isset($_POST['update'])){
    ...
    if(!empty($ho) && !empty($ten) && $gt!=0 && !empty($gmail)  && !empty($address) && !empty($phone) && empty($pass) && empty($image)){
        $upate="UPDATE dangki SET ho='$ho' , ten='$ten' , gt='$gt' , email='$gmail', phone='$phone' , level='1', address='$address' WHERE id='$id'";
    ...
    elseif(!empty($ho) && !empty($ten) && $gt!=0 && !empty($gmail) && !empty($pass) && !empty($image) && !empty($address) && !empty($phone)){
        $pass=md5($pass);
        $repass=md5($repass);
        if($pass != $repass ){
            echo '<script language="javascript">alert("Mật khẩu nhập lại không đúng!");</script>';
        }else{
            $upate="UPDATE dangki SET ho='$ho' , ten='$ten' , gt='$gt' , email='$gmail',phone='$phone', image='$image', level='1' , pass='$pass' , address='$address' WHERE id='$id'";
    ...
    elseif(!empty($ho) && !empty($ten) && $gt!=0 && !empty($gmail) && !empty($address) && !empty($phone) && !empty($pass) && empty($image)){
        $pass=md5($pass);
        $repass=md5($repass);
        if($pass != $repass){
            echo '<script language="javascript">alert("Mật khẩu nhập lại không đúng!");</script>';
        }else{
            $upate="UPDATE dangki SET ho='$ho' , ten='$ten' , gt='$gt' , email='$gmail',phone='$phone' , level='1' , pass='$pass' , address='$address' WHERE id='$id'";
            // if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
            //     echo '<script language="javascript">alert("Đã upload thành công!");</script>';
            //     }else{
            //     echo '<script language="javascript">alert("Đã upload thất bại!");</script>';
    ...
    elseif(!empty($ho) && !empty($ten) && $gt!=0 && !empty($gmail) && empty($pass) && !empty($address) && !empty($phone) && !empty($image)){
        if($pass != $repass ){
            echo "Mật khẩu nhập lại không đúng!!!";
        }else{
            $upate="UPDATE dangki SET ho='$ho' , ten='$ten' , gt='$gt' , email='$gmail',phone='$phone' , level='1',image='$image' , address='$address' WHERE id='$id'";
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
                echo '<script language="javascript">alert("Đã upload thành công!");</script>';
                // header('location: shop.php?check=infor_user');
                }else{
